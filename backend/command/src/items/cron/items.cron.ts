...
async correctAllItemTagCounts() {
  ...
  try {
    ...
    this.logger.log(
      `Cronjob for updating Tag count in Items finished in${performance.now() - updateTagCountStart
      } (Job failed)`,
    )
    return
  }
  this.logger.log(
    `Cronjob for Tag count in Items finished in ${performance.now() - updateTagCountStart
    } (Job succeeded)`,
  )
}
...
async correctAllItemsByTagCounts() {
  ...
  try {
    ...
    this.logger.error(error)
    this.logger.log(
      `Cronjob for updating Tag Counts in ItemsByTag finished in ${performance.now() - updateTagCountStart
      } (Job failed)`,
    )
    return
  }
