...
function Home({ items, statistics }: InferGetServerSidePropsType<typeof getServerSideProps>) {
	...
	const totalItems = statistics.totalItems.toLocaleString("en-US", {
		maximumFractionDigits: 0,
	})
	const totalUsers = statistics.totalUsers.toLocaleString("en-US", {
		maximumFractionDigits: 0,
	})
	...
	return (
		<div>
			...
			<header
				style={{ backgroundImage: `url(${backgroundHeader.src})` }}
				className="bg-background-header-index bg-no-repeat bg-cover bg-center md:border-t-4 md:border-blue-500"
			>
				{/* Navbar */}
				<Navbar />
			...
			<main className="bg-gray-100 pt-5 md:pt-10">
				...
				<div className="container flex flex-col items-center mb-5 md:mb-10">
					...
					<p className="text-gray-600 text-center mb-4 md:mb-8">
						World Wide Weights is a website where you can discover the weights of all the items you can imagine. Explore the largest database of weights!
					</p>
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-2 md:gap-5 mb-5 md:mb-8 w-full">
						{items.map((item) => (
							<ItemPreviewGrid key={item.slug} {...item} imageUrl={getImageUrl(item.image)} />
						))}
					</div>

					<Button className="mb-2" to={routes.weights.list()} icon="weight">
						Show all weights
					</Button>
				</div>
				...
				<div className="bg-blue-900 py-10">
					<div className="container flex flex-col items-center">
						<h1 className="text-white text-3xl md:text-4xl text-center font-bold mb-1">
							Search for over <span className="text-blue-300">{totalItems}+ weights</span>
						</h1>
