...
export default function WeightsSingle({ item, relatedItems }: InferGetServerSidePropsType<typeof getStaticProps>) {
	...
	if (currentTabIndex === -1 && currentTab) return <Custom404 />
	...
	return (
		<>
			...
			<Seo
				...
				description={`The weight of ${item.name} is ${weightString}. ${
					item.tags.length
						? `Discover more weights with topics like ${item.tags.map((tag) => tag.name).join(", ")}`
						: "Discover more weights in the world largest database about weights!"
				}.`}
				ogImage={item.image}
				ogImageHeight="512px"
				ogImageWidth="512px"
			...
			<main className="container mt-4 md:mt-10">
				<div className="bg-white rounded-lg px-3 md:px-6 py-4 md:py-8">
					<div className="grid grid-cols-[120px_1fr] md:grid-cols-[250px_1fr] items-center lg:grid-cols-2 mb-5 md:mb-10">
						...
						<div className="flex flex-col lg:col-start-1 lg:col-end-3 pl-5 lg:pl-0 md:mt-5">
							<a
								...
								title={item.name}
								className="flex items-center text-gray-700 w-40 sm:w-80 lg:w-[35rem] 2xl:w-[50rem] truncate text-lg sm:text-2xl md:mb-2"
								href={`https://www.google.com/search?q=${item.name}`}
							>
								{item.name}
								<Icon className="ml-2">open_in_new</Icon>
							</a>
							<Headline title={generateWeightString(item.weight)} size="text-2xl sm:text-4xl lg:text-5xl w-40 sm:w-80 lg:w-[35rem] 2xl:w-[50rem] truncate">
								{weightString}
							</Headline>
						</div>
						...
						<div className="flex flex-col self-start col-start-1 col-end-3 lg:row-start-2 mt-5 lg:mt-0">
							...
							<ul className="flex md:flex-wrap overflow-y-auto">
								<li>
									<div className="md:hidden absolute bg-gradient-to-r right-0 from-transparent to-gray-100 w-20 h-8 py-1"></div>
								</li>
								{item.tags.map((tag, index) => (
									<li key={tag.name} className={`${index === item.tags.length - 1 ? "mr-20" : ""}`}>
					...
					<div className="flex justify-end">
						{isAllowedToEdit && (
							<Button kind="secondary" to={routes.contribute.edit(item.slug)} className="mb-4">
								Suggest an edit
							</Button>
						)}
					</div>
					...
					<div>
						<Tabs selectedTabIndex={!currentTab ? 0 : currentTabIndex}>
							{singleWeightTabs.map((singleWeightTab) => (
								<Tab
									key={singleWeightTab.slug}
									title={singleWeightTab.title}
									link={routes.weights.single(item.slug, {
										tab: singleWeightTab.slug,
									})}
								>
									<div className="bg-gray-100 rounded-lg py-4 px-3 md:px-4">{singleWeightTab.content}</div>
								</Tab>
							))}
						</Tabs>
					</div>
