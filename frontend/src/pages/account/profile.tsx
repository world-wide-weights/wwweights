...
const Profile: NextPageCustomProps = () => {
	...
	const [contributions, setContributions] = useState<PaginatedResponse<Item>>({
		data: [],
		total: 0,
		page: 1,
		limit: 10,
	...
	const onSubmitDelete = async (values: DeleteItemDTO): Promise<void> => {
		...
		try {
			...
			await commandRequest.post(
				`/items/${selectedContribution.slug}/suggest/delete`,
				{
	...
	if (isLoadingProfile) return <SkeletonLoadingProfile />

	return (
		<>
			<Seo title="My Profile" description="Your profile page. Here you can see your contributions and statistics." />
				...
				<div className="lg:flex gap-4">
					<div className="sm:flex lg:flex-col gap-3 2xl:w-1/4 mb-6 lg:mb-0">
						...
						<div className="flex flex-col justify-center lg:justify-start sm:w-1/2 md:w-auto items-center bg-white rounded-lg py-6 px-4 mb-3 sm:mb-0">
							...
							<Headline datacy="profile-username" className="w-64 text-center break-words" level={3} hasMargin={false}>
								{profile.username}
							</Headline>
							<p datacy="profile-registered-since">
					...
					<div className="lg:w-3/4">
						{contributions.data.length === 0 ? (
							...
							<>
								<Headline level={4}>
									Contributions <small className="font-normal">{contributions.total}</small>
								</Headline>
											...
											actions={
												<>
													...
													<Tooltip content="Delete">
														<IconButton
															datacy={`profile-delete-contribution-${contribution.slug}`}
															icon="delete"
								...
								<Pagination
									...
									baseRoute={routes.account.profile}
									itemsPerPage={contributions.limit}
								/>
							</>
						)}
						...
						{({ dirty, isValid, isSubmitting }) => (
							<Form>
								...
								<div className="flex md:justify-between flex-col md:flex-row">
									<Button datacy="profile-cancel-button" kind="tertiary" type="button" onClick={closeDeleteModal} className="my-4 md:my-0">
										Oops, never mind
									</Button>
									<Button datacy="profile-delete-button" kind="primary" type="submit" disabled={!(dirty && isValid)} loading={isSubmitting} icon="delete">
										Delete forever
									</Button>
								</div>
							</Form>
