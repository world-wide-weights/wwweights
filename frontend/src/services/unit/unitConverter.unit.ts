...
describe("UnitConverter", () => {
    ...
    describe("test main function: convertWeightIntoUnit", () => {
        it("should convert value 1 with any unit into any other unit", () => {
            expect(convertWeightIntoUnit(1, "Pg", "mg")).deep.equal({ value: 1e18, unit: "mg" })
            expect(convertWeightIntoUnit(1, "T", "ng")).deep.equal({ value: 1e15, unit: "ng" })
            expect(convertWeightIntoUnit(1, "kg", "mg")).deep.equal({ value: 1e6, unit: "mg" })
            expect(convertWeightIntoUnit(1, "T", "Tg")).deep.equal({ value: 1e-6, unit: "Tg" })
            expect(convertWeightIntoUnit(1, "mg", "pg")).deep.equal({ value: 1e9, unit: "pg" })
            expect(convertWeightIntoUnit(1, "pg", "T")).deep.equal({ value: 1e-18, unit: "T" })
            expect(convertWeightIntoUnit(1, "kg", "µg")).deep.equal({ value: 1e9, unit: "µg" })
        })

        it("should convert any values with any unit into any other unit", () => {
            expect(convertWeightIntoUnit(100, "Pg", "mg")).deep.equal({ value: 1e20, unit: "mg" })
            expect(convertWeightIntoUnit(0.23, "T", "ng")).deep.equal({ value: 2.3e14, unit: "ng" })
            expect(convertWeightIntoUnit(1444423, "kg", "mg")).deep.equal({ value: 1.444423e12, unit: "mg" })
            expect(convertWeightIntoUnit(5, "T", "Tg")).deep.equal({ value: 5e-6, unit: "Tg" })
            expect(convertWeightIntoUnit(7.45, "mg", "pg")).deep.equal({ value: 7.45e9, unit: "pg" })
            expect(convertWeightIntoUnit(836.343, "kg", "µg")).deep.equal({ value: 8.36343e11, unit: "µg" })
            expect(convertWeightIntoUnit(1.234, "g", "g")).deep.equal({ value: 1.234, unit: "g" })
            expect(convertWeightIntoUnit(1.234e1, "g", "g")).deep.equal({ value: 12.34, unit: "g" })
            expect(convertWeightIntoUnit(1.234e-2, "T", "T")).deep.equal({ value: 0.01234, unit: "T" })
        })
    })
})
