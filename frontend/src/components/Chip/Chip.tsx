...
export const Chip: React.FC<ChipProps> = ({ children, to, onClick, iconEnd, hasMargin = true, iconStart, datacy, disabled, dimOpacityWhenDisabled = true, color = "blue" }) => {
	if (onClick && to) return <p>Use &quot;onClick&quot; prop or &quot;to&quot; prop not both!</p>
		...
		<>
			...
			{!to && (
				<button
					...
					disabled={disabled}
					onClick={onClick}
					className={`inline-flex items-center bg-${color}-500 bg-opacity-20 ${
						disabled && dimOpacityWhenDisabled ? "text-opacity-60" : ""
					} text-${color}-600 rounded-full whitespace-nowrap px-5 py-1 mr-2 ${hasMargin ? "mb-2" : ""}`}
				>
					{content}
				</button>
			)}
			...
			{to && (
				<Link
					...
					className={`inline-block bg-${color}-500 ${disabled ? "cursor-default" : ""} ${
						disabled && dimOpacityWhenDisabled ? "text-opacity-60" : ""
					} bg-opacity-20 text-${color}-600 rounded-full whitespace-nowrap px-5 py-1 mr-2 ${hasMargin ? "mb-2" : ""}`}
				>
					{content}
				</Link>
			)}
