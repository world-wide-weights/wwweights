...
export const BaseError: React.FC<BaseErrorProps> = ({ headline, children, backButtonTo = "/", backButtonText = "Back home", ctaContent }) => {
	return (
		<>
			<Seo title={headline} description="An error occured! Please try again later." />
			<main className="container flex items-center min-h-[calc(100vh-88.5px-102.5px)]">
				{/* 100vh page - 88.5px Navbar - 102.5px Footer*/}
				<div className="md:w-2/3">
					<Headline>{headline}</Headline>
					{children}
					<div className="flex items-center mt-5">
						<Button to={backButtonTo} icon="arrow_back">
							{backButtonText}
						</Button>
						{ctaContent}
					</div>
