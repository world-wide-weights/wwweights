    ...
    return <li className={`${bgColor} rounded-lg py-4 px-2 md:px-0 md:py-2 mb-2`}>
        <Link onClick={disableLink ? (event) => event.preventDefault() : () => ""} datacy={datacy} className={`${disableLink ? "cursor-default" : ""} flex flex-col md:flex-row md:items-center md:h-12 mx-2 md:mx-4`} href={disableLink ? "#" : routes.weights.single(slug)}>
            <div className="flex justify-between items-center h-12 md:w-[40%]">
                {/* Name */}
                <h5 datacy="itempreviewlist-name" title={name} className={`${selectedItem ? "font-bold" : "text-gray-600 font-medium"} w-72 sm:w-48 md:w-56 truncate pr-3`}>{name}</h5>
            ...
            <div className={`flex items-center ${hasDifference && selectedItem ? "justify-end" : ""} md:w-[60%]`}>
                ...
                {/* Weight */}
                <h5 datacy="itempreviewlist-weight" title={`${name} has a weight of ${weightString}`} className={`${selectedItem ? "text-blue-500" : "text-gray-800"} text-right font-bold w-52 lg:w-48 pl-5 truncate ${hasDifference ? selectedItem ? "w-2/3 md:w-1/4" : "w-3/6 md:w-1/4" : "w-1/3"} mr-4`}>{weightString}</h5>
