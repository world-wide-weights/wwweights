 ...
 */
export const ItemPreviewList: React.FC<ItemPreviewListProps> = ({
	slug,
	name,
	weight,
	heaviestWeight,
	...
	return (
		<li className={`${bgColor} rounded-lg py-4 px-2 md:px-0 md:py-2 mb-2`}>
			<Link
				onClick={disableLink ? (event) => event.preventDefault() : () => ""}
				datacy={datacy}
				className={`${disableLink ? "cursor-default" : ""} flex flex-col md:flex-row md:items-center md:h-12 mx-2 md:mx-4`}
				href={disableLink ? "#" : routes.weights.single(slug)}
			>
				<div className="flex justify-between items-center h-12 md:w-[40%]">
					{/* Name */}
					<h5 datacy="itempreviewlist-name" title={name} className={`${selectedItem ? "font-bold" : "text-gray-600 font-medium"} w-72 sm:w-48 md:w-56 truncate pr-3`}>
						{name}
					</h5>
					...
					<div className="min-w-[48px] w-[48px]">
						{imageUrl && (
							<Image
								datacy="itempreviewlist-image"
								className="object-cover rounded-lg w-12 h-12 md:mr-5"
								alt={`Image of ${name}`}
								src={imageUrl}
								width={96}
								height={96}
					...
					{hasDifference && !selectedItem && (
						...
						>
							...
							<Icon datacy="arrow-icon" className="text-xl">
								{difference === 0 ? "remove" : difference >= 0 ? "arrow_upward" : "arrow_downward"}
							</Icon>
						</div>
					)}
