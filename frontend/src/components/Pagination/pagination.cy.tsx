...
describe('Pagination', () => {
    describe('Desktop', () => {
        describe('Mount with first page', () => {
            beforeEach(() => {
                cy.mount(<Pagination totalItems={TOTAL_ITEMS} currentPage={1} baseRoute={testRoute} itemsPerPage={ITEMS_PER_PAGE} query="" />)
            })
        ...
        it('should not display component when not enough items', () => {
            cy.mount(<Pagination totalItems={10} currentPage={1} baseRoute={testRoute} itemsPerPage={10} query="" />)

            cy.dataCy('pagination').should('not.exist')
        })

        it('should disable next button when on last page desktop', () => {
            cy.mount(<Pagination totalItems={TOTAL_ITEMS} currentPage={10} baseRoute={testRoute} itemsPerPage={ITEMS_PER_PAGE} query="" />)
    ...
    describe('Mobile', () => {
        ...
        describe('Mount with first page', () => {
            beforeEach(() => {
                cy.mount(<Pagination totalItems={3} currentPage={1} baseRoute={testRoute} itemsPerPage={1} query="" />)
            })
        ...
        it('should not show next button when on page 1 mobile', () => {
            cy.mount(<Pagination totalItems={3} currentPage={3} baseRoute={testRoute} itemsPerPage={1} query="" />)
