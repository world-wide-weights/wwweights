	...
	return (
		<>
			<ul datacy="pagination" className="flex items-center justify-center">
				...
				<li>
					<Button
						datacy="pagination-button-previous"
						to={paginationService.prev ?? " "}
						disabled={!paginationService.prev}
						icon="arrow_back_ios_new"
						className={`${!paginationService.prev ? "hidden" : "flex"} sm:hidden md:flex mr-5`}
						kind="tertiary"
					>
						Previous
					</Button>
				...
				{paginationService.pages.map((page, index) =>
					...
					page.content === Ellipsis ? (
						<li datacy="pagination-dots" key={index} className="hidden sm:list-item text-gray-500">
							&#8230;
						</li>
					) : (
						<li className="hidden sm:list-item" key={index}>
							<Button
								datacy={`pagination-button-page-${page.content}`}
								to={page.link}
								className={`flex justify-center items-center rounded-full min-w-[36px] w-9 h-9 ${
									page.content === currentPage ? "bg-blue-500 text-white hover:text-white focus:text-white" : "hover:bg-gray-200"
								}`}
								kind="tertiary"
							>
				...
				<li>
					<Button
						datacy="pagination-button-next"
						to={paginationService.next ?? " "}
						disabled={!paginationService.next}
						icon="arrow_forward_ios"
						iconSlot="end"
						className={`${!paginationService.next ? "hidden" : "flex"} sm:hidden md:flex ml-5`}
						kind="tertiary"
					>
						Next
					</Button>
