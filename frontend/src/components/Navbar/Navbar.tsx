			...
			<nav className="container md:flex justify-between">
				...
				<ul className={`${isNavMobileOpen ? "block" : "hidden"} md:flex items-center gap-4 py-5 md:py-0`}>
					{!isLoading &&
						navLinks.map(
							(navLink) =>
								navLink.shouldDisplay && (
									<li key={navLink.text} className="mb-4 md:mb-0">
										<Button
											{...navLink}
											isColored={"to" in navLink && navLink.to === router.pathname}
											disabled={"to" in navLink && navLink.to === router.pathname}
