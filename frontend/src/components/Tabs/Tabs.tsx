			...
			<div className="flex flex-wrap items-center">
				...
				{children.map(({ props: { title, link } }, index) =>
					selectedTabIndex === index ? (
						<Chip datacy={`tab-chip-${index}`} disabled dimOpacityWhenDisabled={false} to={link} key={index}>
							{title}
						</Chip>
					) : (
						<Button datacy={`tab-link-${index}`} key={index} to={link} kind="tertiary" className="px-5 py-1 mr-2 mb-2" shallow={true}>
							{title}
						</Button>
					)
				)}
