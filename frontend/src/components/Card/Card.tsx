...
export const Card: React.FC<CardProps> = ({ icon, descriptionTop, value, descriptionBottom, to, classNameWrapper = "", noTruncate = false }) => {
	...
	return (
		<CustomTag href={to ?? "#"} className={`flex items-center bg-white rounded-lg py-4 px-6 ${classNameWrapper}`}>
			{/* Icon */}
			<div className="flex items-center justify-center bg-blue-200 rounded-full w-14 h-14 min-w-[56px] mr-6">
				<Icon className="text-blue-700 text-3xl" isFilled>
					{icon}
				</Icon>
			</div>
			...
			<div>
				{descriptionTop && (
					<div title={descriptionTop} className={`font-medium text-gray-600 ${noTruncate ? "" : " w-40 truncate break-words"}`}>
						{descriptionTop}
					</div>
				)}
				<h5 title={value} className={`font-bold text-xl  ${noTruncate ? "" : "w-32 truncate"} leading-6`}>
					{value}
				</h5>
