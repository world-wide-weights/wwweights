...
export const Button: React.FC<ButtonProps> = ({
	...
	const innerContent = (
		<>
			{icon &&
				iconSlot === "start" &&
				(loading ? <Icon className={`text-xl mr-2 ${loading ? "animate-spin" : ""}`}>sync</Icon> : <Icon className="text-xl mr-1">{icon}</Icon>)}
			{children}
		...
		<>
			...
			{kind !== "tertiary" && to && (
				<Link
					...
					className={`${buttonBaseClasses} ${kind === "primary" ? primaryClasses : secondaryClasses} ${className}`}
					shallow={shallow}
				>
					{innerContent}
				</Link>
			...
			{kind === "tertiary" && !to && (
				<button datacy={datacy} disabled={disabled} onClick={disabled ? () => "" : onClick} type={type} className={`${linkBaseClasses} ${disabledTertiary} ${className}`}>
					{innerContent}
				</button>
			)}
		</>
