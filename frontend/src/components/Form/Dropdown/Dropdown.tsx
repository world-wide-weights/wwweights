...
export const Dropdown: React.FC<DropdownProps> = ({ name, placeholder, options }) => {
    ...
    return <Field name={name}>{(props: FieldProps<any>) => (
        <div className="relative mb-1">
            ...
            <button datacy={`${name}-dropdown-button`} className={`relative flex items-center justify-between rounded-lg mr-5 sm:mb-0 py-2 px-5 w-full border border-gray-200 bg-gray-200 ${!props.field.value ? "text-gray-500" : "text-gray-800"}`} type="button" onClick={() => setIsOpen(!isOpen)}>
