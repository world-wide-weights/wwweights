			...
			{(arrayHelpers) => {
				...
				const handleKeyUp = (event: React.KeyboardEvent<HTMLInputElement>): void => {
					...
					chipValues.forEach((chip) => {
						...
						if (values[name].includes(trimmedChip)) return
				...
				const handleKeyDown = (event: React.KeyboardEvent<HTMLInputElement>) => {
					if (event.key === "Backspace" && event.currentTarget.value === "") arrayHelpers.remove(values[name].length - 1)
				}
				...
				return (
					<div datacy="chiptextinput-wrapper" className="mb-2 md:mb-4">
						...
						<div
							...
							className="cursor-text flex flex-wrap gap-y-2 border-2 border-gray-100 bg-gray-100 focus-within:outline focus-within:outline-2 focus-within:outline-blue-500 rounded-lg px-2 py-1 mb-2"
						>
							{/* Chips */}
							{values[name] &&
								values[name].map((chip: string, index: number) => (
									<Fragment key={index}>
										<Field name={`${name}.${index}`}>
											{(props: FieldProps) => (
												<>
													...
													<Chip
														datacy={`chiptextinput-chip-${index}`}
														key={index}
														hasMargin={false}
														iconEnd="close"
														color={props.form.errors[name]?.[index as keyof typeof props.form.errors.name] ? "red" : undefined}
							...
							<input
								ref={inputRef}
								datacy={`chiptextinput-${name}-text-input`}
								className="focus-visible:outline-none placeholder:text-gray-400 border-2 border-gray-100 bg-gray-100 py-1"
						...
						{/* Helpertext */}
						{!errors[name] && (
							<p datacy="chiptextinput-helpertext" className="text-gray-600 text-sm mt-2">
								{helperText}
							</p>
						)}

						{/* Error Messages */}
						{(errors[name] as string[])?.map(
							(error: string, index: number) =>
								error && (
									<div datacy="chiptextinput-error" key={index} className="flex items-center text-red-500">
