				...
				{(props: FieldProps<any>) => (
					<>
						<div datacy={`textinput-${name}-inputwrapper`} className="relative flex flex-col justify-center my-1">
							{icon && !iconLink && !iconOnClick && (
								<Icon className={`absolute right-4 ${props.meta.error && props.meta.touched ? "text-red-500" : "text-gray-500"} `}>{icon}</Icon>
							)}
							{icon && iconLink && !iconOnClick && (
								<Link href={iconLink}>
									<Icon className={`absolute right-0 px-4 py-2 ${props.meta.error && props.meta.touched ? "text-red-500" : "text-gray-500"}`}>{icon}</Icon>
								</Link>
							)}
							{icon && iconOnClick && !iconLink && (
								<button type={iconButtonIsSubmit ? "submit" : "button"} onClick={iconOnClick}>
									<Icon
										datacy={`text-input-icon-${name}`}
										className={`absolute right-0 px-4 py-[10px] ${props.meta.error && props.meta.touched ? "text-red-500" : "text-gray-500"}`}
									>
										{icon}
									</Icon>
								</button>
							...
							<input
								...
								{...restProps}
								{...props.field}
								className={`rounded-lg pl-4 py-2 ${icon ? "pr-12" : ""} placeholder:text-gray-400 ${
