            ...
            <Field type={type} name={name}>{(props: FieldProps<any>) => (
                <>
                    <div datacy={`textinput-${name}-inputwrapper`} className="relative flex flex-col justify-center my-1">
                        ...
                        <input datacy={`textinput-${name}-input`} {...restProps} {...props.field} className={`rounded-full pl-4 py-2 ${icon ? "pr-12" : ""} placeholder:text-gray-400 ${props.meta.error && props.meta.touched ? 'bg-red-500 bg-opacity-10 border-2 border-red-500 focus:outline-none focus:border-red-500 focus:ring-red-500' : 'border-2 border-gray-100 bg-gray-100'}`} />
                    </div>
