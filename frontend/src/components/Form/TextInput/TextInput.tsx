            ...
            <Field type={type} name={name}>{(props: FieldProps<any>) => (
                <>
                    <div datacy={`textinput-${name}-inputwrapper`} className="relative flex flex-col justify-center my-1">
                        ...
                        {icon && iconOnClick && !iconLink && <button type={iconButtonIsSubmit ? "submit" : "button"} onClick={iconOnClick}><Icon datacy={`text-input-submit-icon-${name}`} className={`absolute right-0 px-4 py-[10px] ${props.meta.error && props.meta.touched ? "text-red-500" : "text-gray-500"}`}>{icon}</Icon></button>}
