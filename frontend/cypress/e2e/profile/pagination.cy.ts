...
describe("Pagination Profile", () => {
	...
	describe("Base tests", () => {
		it("should display pagination", () => {
			cy.login({
				route: "/account/profile?page=1&limit=2",
			})
		...
		it("should show page 2 when query is page=2", () => {
			cy.login({
				route: "/account/profile?page=2&limit=2",
			})
		...
		describe("Errors", () => {
			it("should show first page when current page is 0 or less", () => {
				cy.login({
					route: "/account/profile?page=0",
				})
				cy.contains("Contribution").should("be.visible")
				cy.login({
					route: "/account/profile?page=-1",
				})
				cy.contains("Contribution").should("be.visible")
			})
			...
			it("should show first page when limit is 0 or less", () => {
				cy.login({
					route: "/account/profile?page=1&limit=0",
				})
				cy.contains("Contribution").should("be.visible")

				cy.login({
					route: "/account/profile?page=1&limit=-1",
				})
				cy.contains("Contribution").should("be.visible")
			})
		})
		...
		describe("Buttons", () => {
			it("should show next page when click next button", () => {
				cy.login({
					route: `/account/profile?page=${currentPage}`,
				})
	...
	describe("Limit", () => {
		beforeEach(() => {
			cy.login({
				route: `/account/profile?page=1&limit=${limit}`,
			})
		})
